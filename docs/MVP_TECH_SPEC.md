...
- Stage 15 — Logos for projects & collaborations (list/detail + owner change) — In Progress
  ...
  - Deliverables to date: server `logoUrl` mapping; shared `LogoImage`; homepage/search/cards/detail wired; submit gating on create; owner-only overlay with headless picker and spinner.
